...
if command -v gh >/dev/null 2>&1; then
  ...
  tab_id="$(herdr pane list 2>/dev/null | HERDR_PANE_ID="$HERDR_PANE_ID" python3 -c 'import sys, json, os
pid = os.environ.get("HERDR_PANE_ID")
try:
    panes = json.load(sys.stdin)["result"]["panes"]
    print(next(p["tab_id"] for p in panes if p.get("pane_id") == pid))
except Exception:
    pass' 2>/dev/null || true)"
  if [ -n "$tab_id" ]; then
    if [ -n "$pr_number" ]; then
      if [ "$pr_state" = "MERGED" ]; then
        herdr tab report-pr "$tab_id" --pr "$pr_number" --merged >/dev/null 2>&1 || true
      else
        herdr tab report-pr "$tab_id" --pr "$pr_number" >/dev/null 2>&1 || true
      fi
    else
      herdr tab report-pr "$tab_id" --clear-pr >/dev/null 2>&1 || true
    fi
  fi
fi
