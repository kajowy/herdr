...
| Code | When it occurs |
| --- | --- |
| `destination_refused` | The resolved upload destination is outside the home directory, a denylisted system or config path, inside a `.git` directory, or the target pane has no resolvable working directory. |
| `invalid_file_path` | A relative path is absolute, nests too deeply, has an empty or otherwise disallowed component, or the checksum is not a sha256 hex digest. |
