...
The same `[server]` table also controls file uploads to this machine: `file_inbox` is the directory that receives uploaded files, `file_max_bytes` and `file_total_max_bytes` cap a single file and a whole connection's transfers, and `file_chunk_bytes` sets how much file data each transfer chunk carries.
