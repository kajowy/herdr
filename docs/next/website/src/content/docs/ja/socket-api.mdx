...
`code` は安定していて分岐に使えます。`message` は人間向けで、リリース間で変わることがあります。

### エラーコード

ソケット API が返しうるエラーコードを、発生源の領域ごとにまとめます。CLI がローカルに表示するだけの
コードと、クライアントシェルのエンドポイント転送に属するコードは含みません。
